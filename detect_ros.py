...
from yolov7_ros.msg import PointCenter  # 추가함
from yolov7_ros.msg import PointCenterArray # 추가함
...
class YoloV7:
    def __init__(self, weights, conf_thresh: float = 0.5, iou_thresh: float = 0.45,
        ...
        self.model.eval()
        self.pointcenter_pub = rospy.Publisher('pointcenter_topic', PointCenterArray, queue_size=10)
    ...
    def inference(self, img: torch.Tensor):
        ...
        point_center_array_msg = PointCenterArray()  # Create an empty PointCenterArray message
        if detections and len(detections) > 0 and detections[0] is not None:
            detections = detections[0].cpu().numpy()  # Convert to numpy array
            for detection in detections:
                if len(detection) >= 4:
                    x1, y1, x2, y2 = detection[:4]
                    xc, yc = self.bboxcenter(x1, y1, x2, y2)
                    point_msg = PointCenter()
                
                    point_msg.x = xc
                    point_msg.y = yc
                    point_center_array_msg.points.append(point_msg)  # Add the PointCenter message to the array
    
        self.pointcenter_pub.publish(point_center_array_msg)  # Publish the PointCenterArray message
        return detections
        
    def bboxcenter(self, x1, y1, x2, y2):
        xc = x1 + ((x2 - x1) / 2)
        yc = y1 + ((y2 - y1) / 2)
        return xc, yc
